...
export function createSpeechBubble(scene, x, y, quote, opts = {}) {
  const padding = opts.padding ?? 10;
  const maxTextWidth = opts.maxTextWidth ?? 260; // max width before wrapping
  ...
  const tempText = scene.make.text({
    ...
    add: false, // <- important: we only use this to measure
  });
  ...
  const bubbleWidth =
    Math.min(textWidth, maxTextWidth) + padding * 2; // don’t exceed max width
  const bubbleHeight = textHeight + padding * 2;
  const arrowHeight = bubbleHeight / 4;

  tempText.destroy(); // we don't need the measurement text anymore
